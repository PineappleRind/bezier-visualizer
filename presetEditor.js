 ...
 ************************/
 ...
 function getPresetListHTML() {
     ...
     for (let i = 0; i < points.presets.length; i++) {
         res += `	<div class="preset"><h1>${points.presets[i].name}</h1>
         <button onclick="removePreset(${i});toast('Preset ${points.presets[i].name} deleted');this.parentElement.remove()">Delete</button>
         <button onclick="editPresetData(${i})">Edit Data</button>
         <button onclick="changePresetName(${i});">Rename</button></div>
         `
     }
     res += `
     </div>`
 ...
 function editPresetData(index) {
     presetEditorPages[1] = `<p id="close" onclick="removeModal()">&times;</p><div>
     <h1>Editing data for ${points.presets[index].name}</h1>
     <textarea style="width:200px; height:300px">${JSON.stringify(points.presets[index].data).replace(/\]\,/g, '],&#13;&#10;').replace(/\[\[/g, '[&#13;&#10;[').replace(/\]\]/g, ']&#13;&#10;]')}</textarea>
     <button onclick="savePresetData(${index},this.previousElementSibling)">Save</button>`
     presetEditorPage(1)
     psc.innerHTML = getPresetSelectHTML()
 }
 ...
 function savePresetData(index, textarea) {
     ...
     toast('Successfully edited preset ' + points.presets[index].name)
 }
 function addPreset() {
 ...
 function parseAndAddPreset(name, data) {
     ...
     try {
         ...
         toast('Invalid data structure')
         return
     }
     if (newData.length < 2) {
         toast('Please add more than 1 point')
         return
     }
     for (let i = 0; i < newData.length; i++) {
         if (newData[i].length != 2) {
             toast('Point ' + (i + 1) + ' does not have proper coordinates.')
         }
     }
